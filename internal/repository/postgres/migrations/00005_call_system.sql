...
-- Adds: calls, call_events, call_participants, call_recordings
-- Demotes: voice_calls (kept as audit-only, app no longer writes)
-- ============================================================
...
-- ============================================================
-- Table: calls
-- Authoritative call state. Single row per call.
-- ============================================================
...
-- ============================================================
-- Table: call_events
-- Append-only audit log. Every state change AND every ARI event lands here.
-- ============================================================
...
-- Idempotent insert: same call+event_type+occurred_at → conflict
...
-- Records explicit actions performed by participants (accept/reject/hangup/cancel).
-- UNIQUE constraint provides DB-level idempotency.
-- ============================================================
...
-- One row per recording. Storage URL filled when ARI RecordingFinished.
-- ============================================================
...
-- Idempotency table for /api/calls POST (cached responses, 24h)
...
CREATE INDEX IF NOT EXISTS idx_call_idempotency_created
    ON call_idempotency(created_at);

-- Backfill historical rows from voice_calls → call_events so we keep audit trail.
INSERT INTO call_events (call_id, event_type, source, agent_id, payload, occurred_at)
SELECT
    md5(vc.session_id)::uuid AS call_id,
    'legacy_voice_call',
    ...
    jsonb_build_object(
        ...
        -- voice_calls has no `status` column (legacy schema from migration
        -- 00001). Derive a synthetic status from `ended_at` so this
...
-- voice_calls is intentionally left in place (audit only).
-- +goose StatementEnd
