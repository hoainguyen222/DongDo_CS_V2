...
func (r *UserRepo) UpdateUser(ctx context.Context, username, fullName string, role domain.UserRole, isActive bool, passwordHash, salt string) (*domain.User, error) {
	if passwordHash != "" {
		row := r.db.Pool.QueryRow(ctx, `
			UPDATE users
			SET full_name = $1, role = $2::user_role, is_active = $3, password_hash = $4, salt = $5
			WHERE LOWER(username) = LOWER($6)
			RETURNING id, username, full_name, role, is_active, created_at
		`, fullName, string(role), isActive, passwordHash, salt, username)
		var u domain.User
		var roleStr string
		err := row.Scan(&u.ID, &u.Username, &u.FullName, &roleStr, &u.IsActive, &u.CreatedAt)
		if err != nil {
	...
	row := r.db.Pool.QueryRow(ctx, `
		UPDATE users
		SET full_name = $1, role = $2::user_role, is_active = $3
		WHERE LOWER(username) = LOWER($4)
		RETURNING id, username, full_name, role, is_active, created_at
	`, fullName, string(role), isActive, username)
