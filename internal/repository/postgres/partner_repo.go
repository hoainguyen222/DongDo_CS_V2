...
func (r *PartnerRepo) ListRolePermissions(ctx context.Context) ([]*domain.RolePermission, error) {
	rows, err := r.db.Pool.Query(ctx, `
		SELECT id, role_name, feature_key, COALESCE(permission_level, 'act'), can_view, can_edit, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.RoleName, &p.FeatureKey, &p.PermissionLevel, &p.CanView, &p.CanEdit, &p.UpdatedAt); err != nil {
			return nil, err
		}
		list = append(list, &p)
	}
	...
	if len(list) == 0 {
		list = getDefaultRolePermissions()
		for _, item := range list {
			_ = r.UpsertRolePermission(ctx, item)
		}
	}
	return list, nil
}
...
func (r *PartnerRepo) UpsertRolePermission(ctx context.Context, p *domain.RolePermission) error {
	...
	canView := p.PermissionLevel != "none"
	canEdit := p.PermissionLevel == "act"
	...
	_, err := r.db.Pool.Exec(ctx, `
		...
		SET permission_level = EXCLUDED.permission_level, can_view = EXCLUDED.can_view, can_edit = EXCLUDED.can_edit, updated_at = NOW()
	`, p.RoleName, p.FeatureKey, p.PermissionLevel, canView, canEdit)
	return err
}

func getDefaultRolePermissions() []*domain.RolePermission {
...
func (r *PartnerRepo) GetAIPerformanceReport(ctx context.Context, startDate, endDate time.Time) (*domain.AIPerformanceMetrics, error) {
	row := r.db.Pool.QueryRow(ctx, `
		SELECT
			...
			COUNT(CASE WHEN status = 'NEEDS_HUMAN_CS' OR status = 'HUMAN_CS_ACTIVE' THEN 1 END)::int AS handoff_cases,
			COALESCE((SELECT AVG(rating)::float FROM csat_feedback WHERE target_type = 'ai' AND created_at >= $1 AND created_at <= $2), 0)::float AS avg_ai_csat
		FROM chat_cases
		WHERE created_at >= $1 AND created_at <= $2
	`, startDate, endDate)
...
func (r *PartnerRepo) GetStaffPerformanceReport(ctx context.Context, startDate, endDate time.Time) ([]*domain.StaffPerformanceItem, error) {
	rows, err := r.db.Pool.Query(ctx, `
		SELECT
			...
			COALESCE((SELECT AVG(cf.rating)::float FROM csat_feedback cf WHERE cf.staff_username = c.assigned_cs AND cf.created_at >= $1 AND cf.created_at <= $2), 0)::float AS avg_csat
		FROM chat_cases c
...
func (r *PartnerRepo) GetCXReport(ctx context.Context, startDate, endDate time.Time) (*domain.CXMetricsReport, error) {
	row := r.db.Pool.QueryRow(ctx, `
		SELECT
			...
			COALESCE(AVG(rating)::float, 0)::float AS avg_csat_score,
	...
	if res.TotalFeedbackCount > 0 {
		res.NSIIndex = fmt.Sprintf("%+.1f%%", (float64(res.PositiveFeedbackCount-res.NegativeFeedbackCount)/float64(res.TotalFeedbackCount))*100.0)
		res.FCRRate = fmt.Sprintf("%.1f%%", (float64(res.PositiveFeedbackCount)/float64(res.TotalFeedbackCount))*100.0)
	} else {
		res.NSIIndex = "0%"
		res.FCRRate = "0%"
	}
...
func (r *PartnerRepo) CreateSystemError(ctx context.Context, errRecord *domain.SystemErrorRecord) (*domain.SystemErrorRecord, error) {
	...
	_, _ = r.db.Pool.Exec(ctx, `DELETE FROM system_errors WHERE created_at < NOW() - INTERVAL '30 days'`)
	...
	`, errRecord.ID, errRecord.Source, errRecord.Title, errRecord.Details, errRecord.Severity, errRecord.IsHandled, errRecord.SuggestedFix, errRecord.CreatedAt)
...
func (r *PartnerRepo) ListSystemErrors(ctx context.Context) ([]*domain.SystemErrorRecord, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Source, &item.Title, &item.Details, &item.Severity, &item.IsHandled, &item.SuggestedFix, &item.CreatedAt); err != nil {
			return nil, err
		}
		list = append(list, &item)
	}
...
func (r *PartnerRepo) MarkSystemErrorHandled(ctx context.Context, id string) error {
	_, err := r.db.Pool.Exec(ctx, `UPDATE system_errors SET is_handled = TRUE WHERE id = $1`, id)
	return err
}
