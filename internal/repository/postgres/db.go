...
func NewDB(ctx context.Context, databaseURL string) (*DB, error) {
	...
	if err := db.RunMigrations(ctx, "db/migrations/001_init.sql"); err != nil {
		log.Printf("⚠️ Migration 001 warning: %v", err)
	}
	if err := db.RunMigrations(ctx, "db/migrations/002_partner_features.sql"); err != nil {
		log.Printf("⚠️ Migration 002 warning: %v", err)
	}
