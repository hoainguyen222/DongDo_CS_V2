...
func (q *Queries) CountCalls(ctx context.Context, dollar_1 string) (int64, error) {
	row := q.db.QueryRow(ctx, countCalls, dollar_1)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
type ListAllCallsParams struct {
	Column1 string `json:"column_1"`
	Limit   int32  `json:"limit"`
	Offset  int32  `json:"offset"`
}

func (q *Queries) ListAllCalls(ctx context.Context, arg ListAllCallsParams) ([]VoiceCall, error) {
	rows, err := q.db.Query(ctx, listAllCalls, arg.Column1, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listAllCallsLegacy = `-- name: ListAllCallsLegacy :many
SELECT id, session_id, caller_type, caller_id, callee_type, callee_id,
       status, duration_seconds, recording_url, transcript, created_at, ended_at
FROM voice_calls
ORDER BY created_at DESC
...
func (q *Queries) ListAllCallsLegacy(ctx context.Context) ([]VoiceCall, error) {
	rows, err := q.db.Query(ctx, listAllCallsLegacy)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
