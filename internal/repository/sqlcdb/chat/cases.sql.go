...
const listCasesPaged = `-- name: ListCasesPaged :many
SELECT id, session_id, guest_id, customer_name, customer_phone,
       status, assigned_cs, last_message, resolution_note, created_at, updated_at
FROM chat_cases
WHERE (
...
type ListCasesPagedParams struct {
	Column1 string `json:"column_1"`
	Column2 string `json:"column_2"`
	Limit   int32  `json:"limit"`
	Offset  int32  `json:"offset"`
...
func (q *Queries) ListCasesPaged(ctx context.Context, arg ListCasesPagedParams) ([]ListCasesPagedRow, error) {
	rows, err := q.db.Query(ctx, listCasesPaged,
		arg.Column1,
		arg.Column2,
		arg.Limit,
		arg.Offset,
	...
	defer rows.Close()
	items := []ListCasesPagedRow{}
	for rows.Next() {
		var i ListCasesPagedRow
		if err := rows.Scan(
			&i.ID,
			&i.SessionID,
			&i.GuestID,
