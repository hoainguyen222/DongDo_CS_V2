...
CREATE UNIQUE INDEX IF NOT EXISTS idx_messages_idempotent
    ON chat_messages(client_msg_id) WHERE client_msg_id IS NOT NULL;
-- +goose StatementEnd
...
DROP TABLE IF EXISTS chat_cases;
...
-- +goose StatementBegin
DROP TYPE IF EXISTS caller_type;
DROP TYPE IF EXISTS call_status;
DROP TYPE IF EXISTS learn_status;
